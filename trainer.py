...
class Md4All(pl.LightningModule):
    ...
    def __init__(self, cfg, is_train):
        ...
        # This is needed otherwise the evaluation of md4allDD would require to load checkpoint 
        # for teacher even if it is not used
        ...
        self.visualizer = Visualizer(self.temp_context, 
                                     self.loss.photometric_weight > 0.0,
                                     self.evaluation_qualitative_res_path, 
                                     self.evaluation_visualization_set,
    ...
    def log_losses(self, is_train):
        ...
        for loss in self.loss.activated_losses:
            self.log(f"losses/avg_{mode}_{loss}", 
                     self.loss.running_avg_loss(loss), 
    ...
    def pose_estimation(self, inputs):
        pose_vec = self.pose_model(
            inputs[('color_aug_pose', 0)], 
            [inputs["color_aug_pose", i] for i in self.temp_context[1:]], 
            inputs['weather_pose'])
        poses = {("pose", frame_id): Pose.from_vec(pose_vec[:, i], self.rotation_mode) 
                 for i, frame_id in zip(range(pose_vec.shape[1]), self.temp_context[1:])}
        return poses
    ...
    def on_test_epoch_end(self):
        ...
        if self.evaluation_quantitative_res_path is not None:
            metrics_file = os.path.join(
                self.evaluation_quantitative_res_path, 
                f"{self.experiment_name}_result_metrics_{time.strftime('%d%B%Yat%H:%M:%S%Z')}.csv")
