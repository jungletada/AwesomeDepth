...
class Evaluator:
    def __init__(self, model, cfg):
        ...
        self.metrics_modes = ('', '_PostProcess', '_gt', '_PostProcess_gt')
        ...
        self.metric_conditions = (
            ...
            'day-rain',
            'night-clear', 
            'night-rain') if cfg.EVALUATION.CONDITION_WISE else ('all-conditions',)

        # Dictionary for metrics in different conditions
        self.metrics = OrderedDict({
            condition: {mode: {metric: 0.0 for metric in self.metrics_keys} 
    ...
    def compute_average_metrics(self):
        ...
        for condition in self.metric_conditions:
            for mode in self.metrics_modes:
                for metric in self.metrics_keys:
                    intermediate_res[condition][mode][metric] = \
                        self.metrics[condition][mode][metric] / self.metrics[condition]['count'] \
                            if self.metrics[condition]['count'] != 0 else 0.0
    ...
    def evaluate_depth(self, batch):
        ...
        for mode in self.metrics_modes:
            compute_depth_metrics(
                gt=batch["depth_gt"], 
                pred=depth_pp if 'PostProcess' in mode else depth, 
