...
class NuScenesDataset:
    def __init__(self, cfg, transform, dataset_version):
        ...
        self.camera_sensors = [self.orientation_to_camera[orientation] 
                                for orientation in cfg.DATASET.ORIENTATIONS] # "CAM_FRONT" only
        ...
        self.original_width = cfg.DATASET.ORIGINAL_SIZE.WIDTH       # 1600 Original width of dataset images
        self.original_height = cfg.DATASET.ORIGINAL_SIZE.HEIGHT     # 900  Original height of dataset images
        self.res_width = cfg.DATASET.AUGMENTATION.RESIZE.WIDTH      # 576  Image resizing width
        self.res_height = cfg.DATASET.AUGMENTATION.RESIZE.HEIGHT    # 320  Image resizing height
        self.scales = cfg.DATASET.SCALES                            # [0, 1, 2, 3]  Scales to use (was never changed)
        self.temp_context = cfg.DATASET.TEMP_CONTEXT                # [0, -1, 1]  Temporal context frame to use (was never changed)
        self.weather_conditions_train = cfg.DATASET.WEATHER.TRAIN   # ['day-clear'] Weather conditions allowed in training set
        self.weather_conditions_val = cfg.DATASET.WEATHER.VAL       # ['day-clear', 'day-rain', 'night-clear', 'night-rain']  Weather conditions allowed in validation set
        self.weather_conditions_eval = cfg.DATASET.WEATHER.TEST     # ['day-clear', 'day-rain', 'night-clear', 'night-rain']  Weather conditions allowed in test set

        self.load_depth_gt = cfg.DATASET.LOAD.GT.DEPTH                  # True, Load ground truth depth in dataloader
        self.load_pose_gt = cfg.DATASET.LOAD.GT.POSE                    # True, Load ground truth pose in dataloader
        self.load_color_full_size = cfg.DATASET.LOAD.COLOR_FULL_SIZE    # False, Load color samples also in full size in dataloader

        if dataset_version in ['trainval', 'mini']:
            self.scenes_weather_info = self.get_scenes_weather_info() # dict as {scene['token]='day-clear'}
    ...
    def get_train_val_indices(self):
        ...
        self.console_logger.info(
            f"Creating train-val split. \nThe training set includes scenes with visibility/weather conditions: "
...
class NuScenesDataModule(pl.LightningDataModule):
    def __init__(self, cfg):
        ...
        # dataset settings: version, height, width, temporal context frame
        ...
        # adverse condition translation for day->night
        ...
        # adverse condition translation for day-clear->day-rain
    ...
    def setup(self, stage: Optional[str] = None):
        resize = data.transforms.Resize(
            size=(self.resized_height, self.resized_width),
            interpolation=torchvision.transforms.InterpolationMode.LANCZOS,
            antialias=True, 
            sample_keys=['color', 'color_aug', 'color_aug_pose', 'K'],
            temp_context=self.temp_context,
            original_size=(self.original_height, self.original_width))

        color_jitter = data.transforms.RandomColorJitter(
            brightness=self.brightness,
            contrast=self.contrast,
            saturation=self.saturation,
            hue=self.hue,
            p=self.color_jitter_prob,
            sample_keys=self.color_jitter_keys, 
            temp_context=self.temp_context)
        
        gaussian_noise = data.transforms.RandomGaussianNoise(
            min_rand=self.gaussian_noise_rand_min,
        ...
        gaussian_blur = data.transforms.RandomGaussianBlur(
            kernel_size=self.gaussian_blur_kernel_size,
            sigma=self.gaussian_blur_sigma,
            p=self.gaussian_blur_prob,
            sample_keys=self.gaussian_blur_keys, 
            temp_context=self.temp_context)
        
        to_tensor = data.transforms.ToTensor(
            sample_keys=['color', 'color_aug', 'color_aug_pose', 'color_original', 'depth_gt'],
            temp_context=self.temp_context)
        ...
        if self.dataset_version == 'full':
            ...
            nuscenes_subset_eval = NuScenesDataSubset(
                nuscenes_dataset=nuscenes_dataset if self.eval_use_val_set else NuScenesDataset(self.cfg, transform, 'test'), 
                mode='eval', 
                eval_use_val_set=self.eval_use_val_set)
        ...
        if self.train_repeat > 1:
            self.train_dataset = Subset(
                nuscenes_subset_train, 
                [train_idx for train_idcs in 
                    [nuscenes_subset_train.indices for _ in range(self.train_repeat)]
    ...
    def test_dataloader(self):
        test_loader = DataLoader(dataset=self.eval_dataset, 
