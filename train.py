...
def train():
    ...
    if cfg.DATASET.NAME == "nuscenes":
        ...
        data_module = RobotCarDataModule(cfg)
    else:
    ...
    save_dir = os.path.join(
        cfg.SAVE.LOG_DIR, 
        time.strftime('%d%B%Yat%H:%M:%S%Z') + '_' + socket.gethostname() + '_' + cfg.EXPERIMENT_NAME
    )
    tb_logger = pl_loggers.TensorBoardLogger(save_dir=save_dir)

    checkpoint_callback = ModelCheckpoint(
        monitor=cfg.SAVE.MONITOR, 
        save_last=cfg.SAVE.LAST, 
        save_top_k=cfg.SAVE.TOP_K, 
        mode='min', 
        dirpath=cfg.SAVE.CHECKPOINT_PATH, 
        filename=cfg.EXPERIMENT_NAME + '-{epoch:02d}')
    
    # early_stop_callback = EarlyStopping(
    #     monitor='metrics_day-clear/abs_rel', 
    #     min_delta=0.00, 
    #     patience=3, 
    #     verbose=False, 
    #     mode='min')
    ...
    trainer.fit(model, data_module, ckpt_path=cfg.LOAD.CHECKPOINT_PATH)
